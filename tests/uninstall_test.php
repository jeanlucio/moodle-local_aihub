...
/**
 * Tests for the local_aihub pre-uninstallation hook.
 *
 * @package    local_aihub
 * @category   test
...
/**
 * Tests for xmldb_local_aihub_uninstall(). Every table in db/install.xml is dropped
 * automatically by core, so the only thing worth exercising here is the one piece of
 * cleanup core does not do for us: personal AI key preferences stored in
 * user_preferences.
 *
 ...
 *
 * @covers ::xmldb_local_aihub_uninstall
 */
final class uninstall_test extends \advanced_testcase {
    ...
    protected function setUp(): void {
        ...
        parent::setUp();
        $this->resetAfterTest();
        require_once($CFG->dirroot . '/local/aihub/db/uninstall.php');
    }

    /**
     * Tests that the uninstall hook deletes only local_aihub-prefixed preferences,
    ...
    public function test_uninstall_with_no_matching_preferences_is_a_noop(): void {
        $this->assertTrue(xmldb_local_aihub_uninstall());
    }
}
