...
| `tests/local/usage_log_test.php` | 8 | Record insert (with `keysource`, empty model nulled); a failed attempt keeping its reason; the failures-only filter on both readers; site-scoped readers excluding personal/untagged rows; the export reader returning every row past the screen's cap; `display_name` naming a person, the system or an unresolved id |
...
| `tests/output/report_test.php` | 7 | Site-keys report rows carry the requesting user's name and correct provider icon, excluding personal-key rows; empty state; failed rows carrying their reason; the failures-only view and its own empty wording; a request with no user attributed to the system; a row written before the failure columns existed still reading as a success; the page rendering end to end |
...
| `tests/behat/report.feature` | 6 | Both attempts of one request listed with the reason the first failed; the failures filter hiding what worked and returning to the full list; a request made outside a session attributed rather than shown as an id; the report downloading as CSV and as Excel |
| **Total** | **11** | |
