...
* 📊 **Administrator usage report:** every provider call served by the **site keys**, across all users, with **failures-only** and **successes-only** filters and **CSV / Excel** download — gated by `local/aihub:viewusage`.
...
* 🧾 **Usage log + retention task:** one row per **provider call**, not per request (user, requesting component, what was generated, provider, model, key tier, outcome and — when it failed — the provider's own reason), and a scheduled task that purges logs past a configurable retention. A request that falls through to another provider therefore leaves a row for each, so a key that never works cannot hide behind whichever one answers.
