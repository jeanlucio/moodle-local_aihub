...
3. **Review site-key usage (admin):** the **AI usage report** (under the AI Hub category, capability `local/aihub:viewusage`) lists every provider call served by the site keys across all users, with CSV and Excel download. Each attempt gets its own row, carrying the outcome and the reason when it failed, and filters for **failures only** or **successes only** answer whether a provider has stopped responding, or narrow the list back to what was actually generated.
